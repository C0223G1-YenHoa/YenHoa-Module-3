...
from customer
join ordered on customer.c_id=ordered.c_id
join order_detail on ordered.o_id=order_detail.o_id
join product on order_detail.p_id=product.p_id;

select customer.c_name
from customer
left join ordered on customer.c_id=ordered.c_id
where ordered.o_id is null;

select ordered.o_id,ordered.o_date, sum(order_detail.od_qty * product.p_price) total
from ordered
join order_detail on ordered.o_id=order_detail.o_id
join product on order_detail.p_id=product.p_id
group by ordered.o_id;