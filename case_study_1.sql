...
new_ngay_ket_thuc datetime, new_tien_dat_coc double, new_ma_nhan_vien int, new_ma_khach_hang int, new_ma_dich_vu int)
begin
if new_ma_hop_dong in (
select ma_hop_dong
from hop_dong hd)
then
signal sqlstate "45000"
set MESSAGE_TEXT = "this id is existed." ;
end if;
insert into hop_dong(ma_hop_dong, ngay_lam_hop_dong, ngay_ket_thuc, tien_dat_coc, ma_nhan_vien, ma_khach_hang, ma_dich_vu)
...
call sp_them_moi_hop_dong(31,20230510,20230515,0,5,2,1);
...
for each row
begin 
if (daydiff(day,new.ngay_ket_thuc,new.ngay_lam_hop_dong)<2) then
signal sqlstate "45000"
set MESSAGE_TEXT = "The contract termination date must be at least 2 days older than the contract date" ;
end if;
insert into `history`(ma_hop_dong, ngay_lam_hop_dong, ngay_ket_thuc, tien_dat_coc, ma_nhan_vien, ma_khach_hang, ma_dich_vu)
value(new.ma_hop_dong, new.ngay_lam_hop_dong, new.ngay_ket_thuc, new.tien_dat_coc, new.ma_nhan_vien, new.ma_khach_hang, new.ma_dich_vu);
end //
delimiter ;

set foreign_key_checks = 0;
insert into hop_dong(ma_hop_dong, ngay_lam_hop_dong, ngay_ket_thuc, tien_dat_coc, ma_nhan_vien, ma_khach_hang, ma_dich_vu)
value(117,20220520,20210515,0,1,3,4);
